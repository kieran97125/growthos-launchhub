...
export async function AppNav() {
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 shadow-[0_10px_35px_rgba(15,23,42,0.06)] backdrop-blur">
        ...
        <nav className="flex min-w-0 flex-wrap gap-2 md:justify-end">
          {navItems.map((item) => (
            <Link
              ...
              href={item.href}
              prefetch={false}
              className={`rounded-full border px-4 py-2 text-sm font-semibold shadow-sm transition focus:outline-none focus:ring-2 focus:ring-sky-300 ${
          ...
          <Link
            href="/logout"
            prefetch={false}
            className="rounded-full border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-semibold text-rose-700 focus:outline-none focus:ring-2 focus:ring-rose-200"
          >
            登出
          </Link>
        </nav>
