...
const settingsItems = [
  ...
  { href: "/settings/team", label: "Kairvo 團隊權限" },
];
    ...
    <nav className="mt-5 flex min-w-0 flex-wrap gap-2">
      {settingsItems.map((item) => (
        <Link
          ...
          href={item.href}
          prefetch={false}
          className="min-w-0 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm transition hover:border-sky-200 hover:bg-sky-50 focus:outline-none focus:ring-2 focus:ring-sky-300"
        >
          {item.label}
        </Link>
      ))}
